...
PARAMETER_NAME = 'Temperature'  # parameter to export

START_DATE = '2026-01-01'
END_DATE = '2026-02-16'

OUTPUT_CSV = 'exported-stream-data.csv'

def expand_timestamps(values: list[int]) -> list[int]:
    if not values:
        return []
    timestamps = [values[0]]
...
def main():
    ...
    with TandemClient(lambda: token) as client:
        ...
        for stream_id in stream_ids:
            ...
            # dates needs to be in milliseconds
            stream_data = client.query_stream_data(default_model_id,
                                                   [stream_id],
                                                   attrs=list(prop_ids),
                                                   from_date=start_date * 1000,
                                                   to_date=end_date * 1000)
            ...
            for item in stream_data:
                ...
                timestamps = expand_timestamps(item.get('t', []))
                values = item.get('v', [])
                count += len(values)
